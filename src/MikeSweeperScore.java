...
public class MikeSweeperScore {
	...
	public void highScore(int time, String name, Difficulty diff)
		...
		if (diff == diff.EASY) {
			files = "easyScores";
			if (!isFull(files)) {
				FileWriter writer = new FileWriter("resources/easyScores.txt", true);
				writer.write(name + "\r\n");
				writer.write(time + "\r\n");
				writer.close();
			}
			...
			files = "medScores";
			if (!isFull(files)) {
				FileWriter writer = new FileWriter("resources/medScores.txt", true);
				writer.write(name + "\r\n");
				writer.write(time + "\r\n");
				writer.close();
			}
			...
			files = "hardScores";
			if (!isFull(files)) {
				FileWriter writer = new FileWriter("resources/hardScores.txt", true);
				writer.write(name + "\r\n");
				writer.write(time + "\r\n");
				writer.close();
			}
	...
	private void isGreater(int newScore, String newName, String files) {
		...
		try {
			...
			FileWriter writer = new FileWriter("resources/" + files + ".txt", false);
